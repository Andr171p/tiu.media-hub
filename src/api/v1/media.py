...
from src.modules.media import CreateUploadDTO, UploadResponse, uploading
...
async def complete_upload(db: DBSession, upload_id: UUID, user: CurrentUser) -> ...:
    _ = await uploading.complete_upload(db, upload_id, user_id=user.id)
    return ...
